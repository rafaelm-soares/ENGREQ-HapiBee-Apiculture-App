...
import { updateApiary } from "src/redux/actions/action-apiary";
...
class UpdateApiary extends Component<Props, State> {
  constructor(props: Props) {
    super(props);
    const id: string = window.location.href.split('/')[5]; // Access id from URL parameters
    const apiary = this.props.apiaries.find((apiary) => apiary.id === parseInt(id, 10));
    console.log(apiary)
    this.state = {
      form: {
        id: apiary?.id,
        name: apiary?.name || "",
        numberOfHives: apiary?.numberOfHives || 0,
        productionGoal: apiary?.productionGoal || '',
        productionType: apiary?.productionType || '',
        municipality: apiary?.location.municipality || '',
        parish: apiary?.location.parish || '',
        place: apiary?.location.place || '',
        latitude: apiary?.location.latitude || 0,
        longitude: apiary?.location.longitude || 0,
        isApproved: apiary?.isApproved === "APPROVED" ? true : false || false,
      },
      isValid: false,
    };
  }
  ...
  handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    this.props.updateApiary(this.state.form);
  };
        ...
        <form className={styles.form} onSubmit={this.handleSubmit}>
          <div className={styles.general}>
            ...
            <Select
              ...
              name={"productionType"}
              placeholder={this.state.form.productionType}
          ...
          <Select
            ...
            name={"productionGoal"}
            placeholder={this.state.form.productionGoal}
...
const mapDispatchToProps = (dispatch: any) => {
  return {
    updateApiary: (apiary: NewApiary) => {
      dispatch(updateApiary(apiary));
    },
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(UpdateApiary);
